...
%% Link-3
trf_viz_link4= make_transform([0.5, 0.5, 0.3], 0.5, .50, 0.3); % Do not specify parent yet: It will be done in the joint
h(7)= link_box([0.010, 0.2, 0.04], trf_viz_link1p2, [0, 0, 1]); 
plot_axes(trf_viz_link1p2, ' ', true, axis_length); % V_{1-2}
%% Link-End-Effector
...
for q1=[linspace(0, -pi/2, 30), linspace(-pi/2, pi/2, 30), linspace(pi/2, 0, 30)]
    set(handle_axes, 'XLim', [-1,1], 'YLim', [-1,1], 'ZLim', [-1,1]);
